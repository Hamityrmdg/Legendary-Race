...
public class ArabaKontrol2 : MonoBehaviour
{
    ...
    {
        if (Input.GetKeyDown(KeyCode.E)) marþ = !marþ;

        if (marþ)
        {
            speed = carSpeed() * hiz_ayari;
            motor = maxMotorGucu * Input.GetAxis("2.V");
            Debug.Log(Input.GetAxis("2.V"));
            float donus = maxDonusAcisi * Input.GetAxis("2.H");

            float ElfrenTork = frenGuc * Mathf.Abs(Input.GetAxis("Jump"));
